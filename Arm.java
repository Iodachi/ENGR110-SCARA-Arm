...
public class Arm
{
    ...
    private double yt;
    private double xt2;     // position of the tool
    private double yt2;
    ...
    public void directKinematic(){
        ...
        double  xa =xj1+(xj2-xj1)/0.5;
        double  ya =yj1+(yj2-yj1)/0.5;
        // distance between joints
        double d = Math.sqrt(Math.pow(xj2-xj1, 2) + Math.pow(yj2-yj1, 2));//
        if (d<2*r){
            valid_state = true;
            // half distance between tool positions
            double  h = Math.sqrt(Math.pow(r, 2)-Math.pow(d*0.5, 2));
            double alpha= Math.atan(yj1-yj2/xj2-xj1);
            // tool position
            xt  = xa + h*Math.cos(Math.PI*0.5-alpha);
            yt  = ya + h*Math.sin(Math.PI*0.5-alpha);
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        // elbows positions
        xj1 = xm1+0.5*(xt-xm1)+Math.cos(Math.atan2(yt-ym1,xt-xm1)-Math.PI/2)*h1;
        yj1 = ym1+0.5*(yt-ym1)+Math.sin(Math.atan2(yt-ym1,xt-xm1)-Math.PI/2)*h1;
        ...
        // elbows positions
        xj2 = xm2+0.5*(xt-xm2)-Math.cos(Math.atan2(yt-ym2,xt-xm2)-Math.PI/2)*h2;
        yj2 = ym2+0.5*(yt-ym2)-Math.sin(Math.atan2(yt-ym2,xt-xm2)-Math.PI/2)*h2;
